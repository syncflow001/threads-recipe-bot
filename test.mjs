...
const 두줄붙임 = 링크넣기(레시피본, [상품이름, 주소, 주소], { 소개: 소개줄, 앞머리: 요리묶음.링크앞머리, ...표기들 })
...
// 보관함 — 홈에서 건진 글을 쟁여 뒀다가 나중에 꺼내 쓴다
{
  ...
  assert.ok(!/꺼내기\(\)|담기\(쟁일것\)|빼기\(죽은것\)/.test(실행기),
...
console.log('통과 — 보관함 검사 19개 + 물림 풀기 2개')
...
// 말투 추천 — 그 분야에서 잘 퍼지는 계정을 찾아 결을 뽑는다
{
  ...
  assert.deepEqual(다듬음, { 정체성: '나', 말투: '반말', 표현: 'ㅋㅋ, ㅠㅠ', 뜻: null, 왜: '까닭' })
...
// 팩 — 계정 하나 = 분야·언어·제휴 세 팩의 조합 (설계서 §11)
{
  ...
  assert.throws(() => 팩.고르기('요리', '한국어', '아마존 재팬'), /아마존 재팬.*제휴팩이 아직 없다/)
  // 계정정보를 그대로 넘기는 길
  ...
  assert.ok(문구없이.startsWith('[광고]'), '광고 표기는 언어팩이 준다')
  ...
  for (const 이름 of 팩.있는언어()) {
    ...
    assert.ok(언어팩.광고표기?.trim(), `${이름} 언어팩에 광고 표기가 없다`)
...
// 되돌리되 **몰아서 하지 않는다** — 몰아서 한 것이 문제였으니 반대로 간다
{
  ...
  await writeFile(join(방, '팔로우장부.주인모름.json'), JSON.stringify(장부))
  ...
  await writeFile(join(방2, '팔로우장부.주인모름.json'), JSON.stringify(장부))
  const 열린것 = []
  const 가짜크로미움 = {
    launch: async () => ({
      newContext: async () => ({
  ...
  await writeFile(join(방3, '팔로우장부.주인모름.json'), JSON.stringify(장부))
  const 안팔로우중 = {
    launch: async () => ({
      newContext: async () => ({
        addCookies: async () => {},
...
// ── 감시 ① 헬스체크 (2026-08-26, 사용자가 시켰다) ──────────────────
{
  ...
  // 멀쩡하면 탈이 없다. 쿠키 주인은 스레드에게 묻는 것이라 여기서는 가짜로 답한다
  const 주인표 = { [쿠키('a')]: 'first.acct', [쿠키('b')]: 'jp' }
  const r = await 헬.한판({ 뿌리: 방, 계정정보: 정보, 주인묻기: async (c) => 주인표[c] ?? null })
  ...
  const r2 = await 헬.한판({ 뿌리: 방, 계정정보: 정보, 쿠키확인: false })
...
// ── 감시 ② 조회수 알리미 ────────────────────────────────────────
{
  ...
  // 사용자가 정한 조건 — **2일 이내 + 조회수 1만 이상**
  assert.equal(알.설정.문턱, 10000)
  assert.equal(알.설정.며칠, 2)
  const 고른것 = 알.알릴것고르기(
    [글('1', 하루전, 12000), 글('2', 하루전, 9999), 글('3', 사흘전, 50000)], { 지금 })
  assert.deepEqual(고른것.map((g) => g.번호), ['1'],
    '문턱 미달(9,999)이나 2일이 지난 글(사흘 전)을 알리면 안 된다')
...
// 계정 만들기 — 안 되는 조합을 골랐을 때 반쪽 계정이 남으면 안 된다
{
  ...
  assert.throws(() => 계정팩({ 분야: '요리', 언어: '일본어', 제휴: '아마존 재팬' }), /아마존 재팬/)
