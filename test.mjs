...
assert.deepEqual(r.수량경고, { 추가됨: [], 빠짐: ['500ml'] }, '재료에 있던 분량이 결과에서 빠지면 알린다')
...
assert.deepEqual(수량대조('참기름 1스푼', '참기름 1스푤'), { 추가됨: [], 빠짐: ['1스푼'] },
  '단위가 깨지면 결과에서는 안 걸린다. 원문 쪽이 사라진 것으로 잡아야 보인다')
assert.deepEqual(수량대조('두부 1모', '두부 2모'), { 추가됨: ['2모'], 빠짐: ['1모'] }, '분량을 바꾸면 양쪽에서 잡힌다')
assert.deepEqual(수량대조('냉면육수 1/2팩', '냉면육수 1/2팩'), { 추가됨: [], 빠짐: [] }, '분수도 그대로면 통과한다')
const 경고결과 = await 재구성({ ...원글, 글타래: ['소면 1인분'] }, { 페르소나, 키: 'k', fetch: 가짜LLM('{"본문":"x","레시피":"소면 2인분"}') })
assert.deepEqual(경고결과.수량경고, { 추가됨: ['2인분'], 빠짐: ['1인분', '500ml'] },
...
assert.deepEqual(수량대조('두부 1모', '두부 1\u200B모'), { 추가됨: [], 빠짐: [] },
  '폭 없는 문자 차이를 분량이 바뀐 것으로 오해하지 않는다')
assert.equal(레시피있나({}), false, '글타래를 못 받은 글도 뺀다')

// 링크는 LLM 이 아니라 우리가 붙인다
import { 대가성문구 } from './src/compose.mjs'
assert.equal(링크넣기('레시피', []), '레시피', '링크가 없으면 문구도 안 붙인다')
const 링크붙임 = 링크넣기('레시피', [{ 이름: '두부', url: 'https://link.coupang.com/a' }, null, '토스 https://t/b'])
assert.equal(링크붙임, `레시피\n\n${대가성문구}\n두부 https://link.coupang.com/a\n토스 https://t/b`)
assert.ok(링크붙임.indexOf(대가성문구) < 링크붙임.indexOf('link.coupang.com'), '문구가 링크보다 앞에 온다')

// 글타래가 나뉠 때 문구와 링크가 떨어지면 고지 없는 조각이 생긴다
const 긴레시피 = 링크넣기(Array.from({ length: 40 }, (_, i) => `${i}번 재료 100g`).join('\n'),
  [{ 이름: '두부', url: 'https://link.coupang.com/a' }])
const 나뉜것 = 나누기(긴레시피)
assert.ok(나뉜것.length > 1, '표본이 실제로 나뉘어야 의미 있는 검사다')
const 링크조각 = 나뉜것.filter((c) => c.includes('link.coupang.com'))
