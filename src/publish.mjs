...
export async function 글올리기({ 본문, 레시피, 미디어 = [] }, { 사용자 = process.env.THREADS_USER_ID, 토큰 } = {}) {
  ...
  const 사진 = 미디어쓸것.find((m) => m.종류 !== '영상')?.url
  const 링크든조각 = (조각) => 링크꼴.test(조각)
  const 미리보기붙음 = !사진 && 조각들.some(링크든조각)
  ...
  try {
    for (const 조각 of 조각들) {
      앞 = await 답글달기(사용자, 조각, 앞, 토큰, { 사진: 링크든조각(조각) ? 사진 : undefined })
      답글번호들.push(앞)
    }
...
async function 답글달기(사용자, 글, 앞번호, 토큰, { 사진, 최대시도 = 6 } = {}) {
  const 몸통 = 사진
    ? { media_type: 'IMAGE', image_url: 사진, text: 글, reply_to_id: 앞번호 }
    : { media_type: 'TEXT', text: 글, reply_to_id: 앞번호 }
  let 마지막
  for (let 번 = 0; 번 < 최대시도; 번++) {
    try {
      // 사진이 붙으면 스레드가 받아 오는 데 시간이 걸린다. 익기 전에 내보내면 거부당한다
      const 컨테이너 = await 담기(사용자, 몸통, 토큰)
      if (사진) await 익기기다리기(컨테이너, { 토큰 })
...
// 스레드 한도는 500자다. 450 으로 잡아 이모지·링크가 늘어나도 걸리지 않게 여유를 둔다.
export const 글자한도 = 450
