...
const 한걸음 = async (사용자, 내용, 토큰) => 내보내기(사용자, await 담기(사용자, 내용, 토큰), 토큰)
...
export async function 글올리기({ 본문, 레시피, 미디어 = [] }, { 사용자 = process.env.THREADS_USER_ID, 토큰 } = {}) {
  ...
  try {
    for (const 조각 of 조각들) {
      앞 = await 답글달기(사용자, 조각, 앞, 토큰)
      답글번호들.push(앞)
    }
    return { 본문번호, 답글번호들, 버린영상 }
  } catch (e) {
    // 본문은 이미 올라갔다. 몇 조각까지 붙었는지 숨기지 않는다
    return { 본문번호, 답글번호들, 버린영상, 답글오류: e.message }
  }
}

// 막 올린 글에 바로 답글을 달면 "미디어를 찾을 수 없음"(4279009) 이 난다.
// 스레드가 그 글을 답글 받을 수 있는 상태로 만드는 데 몇 초 걸린다 — 특히 캐러셀·영상이 그렇다.
async function 답글달기(사용자, 글, 앞번호, 토큰, 최대시도 = 6) {
  let 마지막
  for (let 번 = 0; 번 < 최대시도; 번++) {
    try {
      return await 한걸음(사용자, { media_type: 'TEXT', text: 글, reply_to_id: 앞번호 }, 토큰)
    } catch (e) {
      마지막 = e
