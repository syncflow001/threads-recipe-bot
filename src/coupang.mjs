...
// orders 리포트 한 건을 다듬는다 — fetch 를 부르지 않는 순수 함수라 표본 하나로 검사할 수 있다
// (docs/coupang-api-facts.md 「orders 엔드포인트 실측」)
export function 주문다듬기(d) {
  return {
    날짜: String(d.date).replace(/^(\d{4})(\d{2})(\d{2})$/, '$1-$2-$3'),
    주문번호: d.orderId,
    상품번호: d.productId,
    상품명: d.productName ?? '',
    수량: Number(d.quantity) || 0,
    거래액: Number(d.gmv) || 0,
    수수료: Number(d.commission) || 0,
    수수료율: Number(d.commissionRate) || 0,
    분류: d.categoryName ?? '',
    꼬리표: d.subId ?? '',
  }
}

// 판매된 상품 리스트 — 실적() 과 같은 날짜 꼴(YYYYMMDD), commission 대신 orders 엔드포인트를 쓴다.
// productName 이 핵심이다 — commission 엔드포인트엔 없다 (docs/coupang-api-facts.md 「orders 엔드포인트 실측」)
export async function 주문목록(시작, 끝, opts = {}) {
  const 날짜꼴 = /^\d{8}$/
  if (!날짜꼴.test(시작) || !날짜꼴.test(끝)) throw new Error('날짜는 YYYYMMDD 여야 한다')
  const data = await 부르기('GET', `${기본}/v1/reports/orders?startDate=${시작}&endDate=${끝}`, null, opts)
  return (data ?? []).map(주문다듬기).sort((a, b) => b.날짜.localeCompare(a.날짜))
}
