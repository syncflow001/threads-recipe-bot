...
// 프로필 문서 한 장에 팔로워 수와 그 계정의 최근 글이 함께 들어 있다.
// 문서를 두 번 받을 까닭이 없다 — 말투 추천이 이 글들로 반응 좋은 글을 고른다.
// 프로필 문서에는 남의 글(추천 글)도 섞여 있어 작성자로 거른다
export async function 프로필읽기(작성자, opts) {
  try {
...
export async function 팔로워수(작성자, opts) {
  return (await 프로필읽기(작성자, opts)).팔로워
}
