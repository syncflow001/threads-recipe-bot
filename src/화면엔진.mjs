...
// 스레드에 적어 둔 **프로필 소개문구**를 그대로 가져온다. 말투 카드의 「나를 한 줄로」가 이것을 쓴다.
//
// ⚠️ **왜 있나 (2026-09-03 사용자 요청).** 페르소나의 정체성과 실제 계정 소개가 어긋나
// **「20대 싱글 여성」 계정이 「3040 주부」 말투로 글을 올리고 있었다.** 소개는 사람이 스레드에
// 직접 적은 것이니 그것을 정본으로 삼는다 — 두 곳에 따로 적으면 언젠가 반드시 어긋난다.
//
// 못 읽으면 **null 을 준다. 지어내지 않는다** — 화면은 그때 저장된 값을 그대로 둔다.
// 쿠키가 없거나 스레드가 얇은 문서를 주는 계정이 있다 (src/읽기쿠키.mjs)
export async function 소개문구(계정) {
  const 그정보 = await 정보(계정).catch(() => null)
  const 아이디 = 보일아이디(계정, 그정보)
  if (!아이디) return { 소개: null, 아이디: null }
  const cookie = await 계정열쇠읽기(계정, 'THREADS_COOKIE')
  const { 프로필읽기 } = await import('./threads.mjs')
  const r = await 프로필읽기(아이디, { cookie }).catch(() => ({ 소개: null }))
  return { 소개: r.소개 ?? null, 아이디 }
}
...
export async function 말투샘플(계정, 카드) {
  ...
  try {
    ...
    const { 분야팩: 묶음 } = 계정팩(그정보)
    알림담기('지금 화면의 말투로 다시 쓰는 중입니다 — 1분쯤 걸립니다', 계정)
    // 샘플도 실제로 나가는 글과 같다 — 2026-09-07 부터 나가는 글은 늘 감추지 않는다 (링크는 터진 글에만 뒤늦게)
    const 글 = await 재구성(원글, { 페르소나: await 임시페르소나(계정, 카드), 묶음, 감추기: false })
    const 값 = {
...
// 말투 추천도 남의 글을 읽기만 한다 — 못 읽으면 다른 계정 로그인을 빌린다.
// 규칙과 까닭은 `src/읽기쿠키.mjs` 머리에 한곳에 적어 두었다. 여기서 또 적지 않는다
...
export async function 말투추천하기(계정) {
  ...
  try {
    ...
    const r = await 말투추천(그정보.분야, {
      ...
      빌릴쿠키들: await 빌릴쿠키들(계정), 빌렸다: (n) => { 빌린계정 = n },
    })
    const 값 = { ...r, 빌린계정 }
    진행끝(값, 계정)   // 새로고침으로 창이 사라졌어도 결과는 남는다
    return 값
