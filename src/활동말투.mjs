...
export async function 활동재료(계정, 그정보, 뿌리 = process.cwd()) {
  const 날것 = await 말투읽기(계정, 뿌리)
  let 팩 = {}
  try { 팩 = (await import('./팩.mjs')).계정팩(그정보) } catch { 팩 = {} }
  // ⚠️ 2026-09-02 — **여기도 서식깔기를 거쳐야 한다.** 이 자리가 말투 파일을 날것으로 쓰고 있었다.
  //    글의 뼈대(글 구조·지켜야 할 것·쓰지 말 것)를 팩으로 옮긴 뒤로 말투 파일에 그 칸이 없어져,
  //    댓글·답글 프롬프트에서 「존댓말 금지」·「꿀팁 금지」·「출처 언급 금지」가 **열한 계정 전부**
  //    통째로 빠졌다. 안 터지고 조용히 빠져서 아무도 몰랐다 (검수 에이전트가 잡았다).
  //    서식깔기는 분야팩이 없으면 페르소나를 그대로 돌려주므로 팩 없는 계정도 안 다친다
  const { 서식깔기 } = await import('./compose.mjs')
  const 페르소나 = 서식깔기(날것, 팩.분야팩 ?? null)
