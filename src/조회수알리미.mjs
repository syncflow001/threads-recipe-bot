...
//
// 조건 (사용자가 정했다) — **잰 때로부터 30시간 이내에 올린 글** 중 **조회수 1만 이상.**
// 옛 글이 천천히 1만을 넘는 것은 알리지 않는다. 「지금 터지고 있는 글」만 본다.
// ⚠️ 2026-09-07 — 창을 2일에서 **30시간**으로 조였다. 이 문턱이 이제 **링크를 다는 조건**이기도 하다
//    (src/늦은링크.mjs). 알림과 링크가 「터진 글」을 다르게 세면 사람이 두 숫자를 기억해야 한다
//
...
export const 설정 = {
  문턱: 10000,   // 조회수 몇부터 알리나 — 그리고 링크를 다나
  시간: 30,      // 잰 때로부터 몇 시간 안에 올린 글만
  글수: 12,      // 계정마다 최근 몇 편까지 볼까 (하루 5편이면 30시간에 넉넉하다)
}
...
// 올린 지 몇 시간 안 됐나
export const 최근인가 = (올린때, 지금, 시간 = 설정.시간) => {
  const t = Date.parse(올린때 ?? '')
  if (!Number.isFinite(t)) return false
  return 지금 - t <= 시간 * 60 * 60 * 1000 && t <= 지금
}
...
export function 알릴것고르기(글들, { 이미알린것 = [], 지금 = Date.now(), 옵션 = 설정 } = {}) {
  ...
  return (글들 ?? []).filter((g) => {
    ...
    if (!최근인가(g.올린때, 지금, 옵션.시간)) return false
...
export async function 계정한판(계정, {
  ...
  // 최근 글만 본다. 창이 지난 글은 성적을 물어볼 것도 없다 — API 를 아낀다.
  // ⚠️ 이미 알린 글도 **잰다** (2026-09-07). 알림은 한 번이지만 링크 달기는 실패하면 다음 판에
  //    다시 해야 한다 — 알림 기록으로 걸러 버리면 그 글은 영영 안 달린다. 알림 여부는 아래에서 가른다
  const 후보 = (await 글목록(계정, { 뿌리 })).slice(0, 옵션.글수)
    .filter((g) => 최근인가(g.올린때, 지금, 옵션.시간))
  ...
  // 잰 것을 통째로 돌려준다 — 링크 달기(늦은링크.mjs)가 알림 기록과 상관없이 문턱을 본다
  return { 계정, 본글: 후보.length, 알릴것, 잰것 }
}
...
export async function 한판({ 뿌리 = process.cwd(), 지금 = Date.now(), 옵션 = 설정, ...나머지 } = {}) {
  ...
  return { 때: new Date(지금).toISOString(), 판들, 알릴것, 정보 }
}
