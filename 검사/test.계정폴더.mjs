// 계정 폴더 규칙을 구조로 못 박는 검사 — node --test 검사/test.계정폴더.mjs
//
...
test('어떤 모듈도 계정 경로를 제 손으로 조립하지 않는다', async () => {
  ...
  for (const 곳 of 볼곳) {
    for (const f of await readdir(곳)) {
      ...
      // 검사 파일은 2026-09-01 에 검사/ 로 갔다. 여기 test. 조건은 이제 안 걸리지만
      // 남겨 둔다 — 뿌리에 test 로 시작하는 파일이 다시 생겨도 규칙 검사에서 빼는 게 맞다
      // (검사는 join(방,'계정','가') 처럼 일부러 경로를 조립한다)
...
// 2026-09-01 — dashboard/app/api 는 test.계정폴더 의 그물 밖이었다. 그 틈으로
// history 가 팔로우장부.주인모름.json 을 뿌리에서 찾다 깨졌고, 아무 검사도 안 잡았다
test('대시보드 라우트가 계정 자료 경로를 제 손으로 조립하지 않는다', async () => {
  const 계정자료 = /(팔로우장부|활동장부|팔로워장부|글성적장부|보관함|persona|열쇠\.env|도달기준선|점검상태|섀도우밴장부)/
  const 걸린것 = []
  const 훑기 = async (곳) => {
    for (const d of await readdir(곳, { withFileTypes: true })) {
      const 길 = join(곳, d.name)
      if (d.isDirectory()) { await 훑기(길); continue }
      if (!d.name.endsWith('.ts')) continue
      const 글 = await readFile(길, 'utf8')
      글.split('\n').forEach((줄, i) => {
        if (줄.trimStart().startsWith('//')) return
        // join(뿌리(), '…계정자료…') 처럼 라우트가 직접 파일 이름을 적는 것을 막는다.
        // 엔진(계정길)을 거치면 이런 줄이 안 나온다
        if (/join\s*\(/.test(줄) && 계정자료.test(줄)) 걸린것.push(`${길}:${i + 1}  ${줄.trim()}`)
      })
    }
  }
  await 훑기('dashboard/app/api')
  assert.deepEqual(걸린것, [],
    '계정 자료 경로는 엔진(계정길)을 거쳐야 한다 — 라우트가 직접 조립하면 자리가 바뀔 때 조용히 깨진다')
})
